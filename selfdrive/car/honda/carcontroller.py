...
from selfdrive.car.honda.values import CruiseButtons, VISUAL_HUD, HONDA_BOSCH, HONDA_NIDEC_ALT_PCM_ACCEL, CarControllerParams, SERIAL_STEERING, LKAS_LIMITS, HYBRID_BRAKE
...
def actuator_hystereses(brake, braking, brake_steady, v_ego, car_fingerprint):
    ...
    # don't change brake command for small oscillations within this value
    brake_hyst_gap = 0.01
...
class CarController():
    ...
    def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd,
        ...
        if active:
            ...
            gas, brake = compute_gas_brake(
                actuators.accel, CS.out.vEgo, CS.CP.carFingerprint)
        else:
            accel = 0.0
            gas, brake = 0.0, 0.0
        ...
        pre_limit_brake, self.braking, self.brake_steady = actuator_hystereses(
            brake, self.braking, self.brake_steady, CS.out.vEgo, CS.CP.carFingerprint)

        # *** rate limit after the enable check ***
        self.brake_last = rate_limit(
            pre_limit_brake, self.brake_last, -2., DT_CTRL)
        ...
        apply_steer = int(interp(actuators.steer * P.STEER_MAX,
                          P.STEER_LOOKUP_BP, P.STEER_LOOKUP_V))

        if (CS.CP.carFingerprint in SERIAL_STEERING):
            apply_steer = apply_std_steer_torque_limits(
                apply_steer, self.apply_steer_last, CS.out.steeringTorque, LKAS_LIMITS, ss=True)
        ...
        if not CS.CP.openpilotLongitudinalControl:
            if (frame % 2) == 0:
                idx = frame // 2
                can_sends.append(hondacan.create_bosch_supplemental_1(
                    self.packer, CS.CP.carFingerprint, idx))
            # If using stock ACC, spam cancel command to kill gas when OP disengages.
            if pcm_cancel_cmd:
                can_sends.append(hondacan.spam_buttons_command(
                    self.packer, CruiseButtons.CANCEL, idx, CS.CP.carFingerprint))
            elif CS.out.cruiseState.standstill:
                can_sends.append(hondacan.spam_buttons_command(
                    self.packer, CruiseButtons.RES_ACCEL, idx, CS.CP.carFingerprint))
            ...
            if (frame % 2) == 0:
                ...
                if CS.CP.carFingerprint in HONDA_BOSCH:
                    self.accel = clip(
                        accel, P.BOSCH_ACCEL_MIN, P.BOSCH_ACCEL_MAX)
                    self.gas = interp(
                        accel, P.BOSCH_GAS_LOOKUP_BP, P.BOSCH_GAS_LOOKUP_V)
                    can_sends.extend(hondacan.create_acc_commands(
                        self.packer, enabled, active, accel, self.gas, idx, stopping, CS.CP.carFingerprint))
                else:
                    apply_brake = clip(self.brake_last - wind_brake, 0.0, 1.0)
                    apply_brake = int(
                        clip(apply_brake * P.NIDEC_BRAKE_MAX, 0, P.NIDEC_BRAKE_MAX - 1))
                    pump_on, self.last_pump_ts = brake_pump_hysteresis(
                        apply_brake, self.apply_brake_last, self.last_pump_ts, ts)
                    if CS.CP.carFingerprint in HYBRID_BRAKE:
                    ...
                    if CS.CP.enableGasInterceptor:
                        ...
                        if active:
                            self.gas = clip(
                                gas_mult * (gas - brake + wind_brake*3/4), 0., 1.)
                        else:
                            self.gas = 0.0
        ...
        if (frame % 10) == 0:
            ...
            can_sends.extend(hondacan.create_ui_commands(
                self.packer, CS.CP, pcm_speed, hud, CS.is_metric, idx, CS.stock_hud))
