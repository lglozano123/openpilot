...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if hud_control.lanesVisible and lat_active:
      hud_lanes = 1
    else:
      hud_lanes = 0
    ...
    idx = self.frame % 4
    can_sends.append(hondacan.create_steering_control(self.packer, apply_steer, lat_active, self.CP.carFingerprint,
    ...
    if self.frame % 10 == 0:
      ...
      hud = HUDData(int(pcm_accel), int(round(hud_v_cruise)) if hud_car != 0 else 255, hud_car,
                    hud_lanes, fcw_display, acc_alert, steer_required, CS.madsEnabled and not lat_active)
